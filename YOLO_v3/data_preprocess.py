...
# 数据预处理：
#       1. 清洗数据
#       2. 重新划分训练集与测试集。从val2014中选取5k张作为测试集，其余作为训练集。
#       
# -----------------------------------------------------------------------------
...
val_file = open('../data/coco/5k.part','w+')
train_file = open('../data/coco/trainvalno5k.part','w+')
count = 0
for i in range(len(val_ids)):
    if count <= 5000:
        ...
        if len(anns) > 0:
            val_file.write('/coco/images/val2014/'+file_name+'\n')
            labels_path = '../data/coco/labels/val2014/'
            if not os.path.exists(labels_path):
                os.makedirs(labels_path)
            lebels_file = open(labels_path + file_name.replace(".png", ".txt").replace(".jpg", ".txt"),'w+')
            for i in range(len(anns)):
                lebels_file.write('%s %s %s %s %s\n'%(anns[i]['category_id'],anns[i]['bbox'][0],anns[i]['bbox'][1],anns[i]['bbox'][2],anns[i]['bbox'][3]))
            lebels_file.close()
            count += 1
        else:
            ...
            anns = val_coco.loadAnns(annids)
            if len(anns) > 0:
                train_file.write('/coco/images/train2014/'+file_name+'\n')
                labels_path = '../data/coco/labels/val2014/'
                if not os.path.exists(labels_path):
                    os.makedirs(labels_path)
                lebels_file = open(labels_path + file_name.replace(".png", ".txt").replace(".jpg", ".txt"),'w+')
